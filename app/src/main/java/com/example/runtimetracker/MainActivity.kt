...
class MainActivity : Activity() {
    ...
    private fun init() {
        ...
        (findViewById<View>(R.id.runs_list) as ListView).onItemClickListener = OnItemClickListener { adapterView, view, index, id ->
            val intent = Intent(this@MainActivity, RunDetails::class.java)
            intent.putExtra(AddEditRunActivity.RUN_ID_STR, id)
            intent.putExtra(AddEditRunActivity.RUN_TIME_STR, (view.findViewById<View>(R.id.txt_run_time) as TextView).text)
            intent.putExtra(AddEditRunActivity.RUN_DIST_STR, (view.findViewById<View>(R.id.txt_run_dist) as TextView).text)
            intent.putExtra(AddEditRunActivity.RUN_DATE_STR, (view.findViewById<View>(R.id.txt_run_date) as TextView).text)
            startActivity(intent)
        }
    ...
    private val runs: Unit
        private get() {
            ...
            try {
                ...
                cursor = db!!.rawQuery("select _id, Run_Date, Run_Time, Run_Distance from Run order by _id desc", null)
                //            SimpleCursorAdapter listAdapter = new SimpleCursorAdapter(this,
//                    R.layout.run_record,
//                    cursor,
//                    new String[]{"Run_Time", "Run_Distance", "Run_Date"},
...
//                    0);
                ...
                Toast.makeText(this, "you're fucking trash: " + ex.message, Toast.LENGTH_LONG).show()
            }
        }
}